...
`TaskFlowDocs/repository-docs/index.md` is a path-only catalog for repository rules, repository guidance, and scoped personal supplements. Repository-owned documents stay at their conventional locations: prefer the repository root for README, contributing, code-style, and roadmap files, and use platform-standard directories for PR templates, CODEOWNERS, CI, and similar files. This applies even before Git is initialized because the directory may become a repository later.

Before a non-trivial task is planned or implemented, TaskFlow refreshes the catalog only when it is absent, stale, or the task needs an uncataloged class, then reads documents applicable to the task phase. The catalog records repository-relative paths and never copies or symbolically links repository-owned documents into `repository-docs/`; that directory contains only `index.md` and optional personal supplements under `personal/`. When explicitly authorized to create a missing document, TaskFlow reuses an established conventional filename or defaults to `README.md`, `CONTRIBUTING.md`, `CODE_STYLE.md`, or `ROADMAP.md` for those classes. Legacy copies or links are reported but never migrated or deleted without explicit authorization.

Repository documents are authoritative. If a needed rule is absent, TaskFlow pauses implementation and guides the user through up to three dependency-ordered questions at a time, with recommendations. Once confirmed, it creates only a scope-matched personal supplement—a person's development rules or work habits—under `repository-docs/personal/`; it cannot weaken, override, or conflict with repository guidance. A material applicable-document change follows the same version gate as any other task-contract change.
