...
Ensure the TaskFlow archive rule is true in both policy and current repository state by recording and archiving all remaining completed historical tasks.
...
- Review found that the Skill description limits automatic selection to non-trivial work, allowing simple requests to bypass Todo-first intake.
- Review found that archive instructions do not require updating the Todo task path atomically, and the archived v4 Plan's top-level status was inconsistent with its completion record.
- The workflow draft retains one obsolete rule saying a goal or deliverable boundary alone creates a new task.
- Audit found two completed task directories still at the active `TaskFlowDocs/` root and without Todo traceability: `2026-09-07-repository-standards-and-doc-root` and `2026-09-07-todo-intake-and-promotion`.
...
11. Ensure the Skill is selected for every repository work request; the body may choose a lightweight path, but Todo-first intake must never be bypassed because a request is small.
12. Treat task archival as one verified transaction: move the complete directory, update the linked Todo item to the achieved path and `done`, then confirm the active path is absent and the achieved task's root documents consistently say `completed`.
13. Remove active documentation wording that makes a changed goal or deliverable automatically a new task when it does not meet the existing-task-first boundary.
14. Create linked Todo records for the remaining completed historical tasks, archive their complete directories, and verify their Todo paths/status and root PRD/Plan statuses.
...
- The Skill description and body make Todo-first intake apply to every repository work request, including a small direct change.
- Completion has one explicit transaction with directory, Todo-path/status, and task-status consistency checks.
- Active references contain no lower-threshold new-task rule.
- No completed task remains at the active root; each archived historical task has a Todo item pointing to its achieved path with status `done`.
...
- Apply the TaskFlow Skill to every repository work request; use a lightweight handling path only after Todo intake.
- Treat archive movement and Todo updates as one verified operation.
- Apply the archive transaction to the two remaining completed historical tasks.
...
- v5 — universal TaskFlow activation and archive-consistency repair.
- v6 — archive remaining completed historical tasks and restore Todo traceability.
