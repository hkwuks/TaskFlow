...
仓库文档优先于个人补充规则。如果任务需要的规则缺失，TaskFlow 会暂停实施，每次最多按依赖顺序询问三个问题并给出建议。用户确认后只在 `repository-docs/personal/` 创建范围匹配的个人补充；个人补充规则指个人的开发规则和工作习惯，不得弱化、覆盖或冲突于仓库文档。适用文档契约的实质变化遵循与其他任务契约相同的版本门禁。
