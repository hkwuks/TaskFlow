...
`TaskFlowDocs/repository-docs/index.md` 是仓库规则、仓库指引和有范围的个人补充规则共用的统一目录。目录统一导航已有 README、贡献指南、代码规范、发布、路线图、PR 和 CI 文档，但不复制源文件内容。
...
仓库文档优先于个人补充规则。如果任务需要的规则缺失，TaskFlow 会暂停实施，每次最多按依赖顺序询问三个问题并给出建议。用户确认后只在 `repository-docs/personal/` 创建范围匹配的个人补充；它不得弱化、覆盖或冲突于仓库文档。适用文档契约的实质变化同样必须先归档旧 Task version、重新批准后才能继续。
...
验证通过后，TaskFlow 将整个任务目录移入 `TaskFlowDocs/achieved/<task-id>/`，同步 Todo 的任务路径和 `done` 状态，并确认活动路径已不存在。之后若新工作属于该已完成交付物，先将目录取回活动根目录，记录 Todo 来源与重新打开原因，创建新的 Task version，并在修改实现前重新经过批准门禁。
