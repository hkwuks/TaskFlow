...
仓库文档优先于个人补充规则。如果任务需要的规则缺失，TaskFlow 会暂停实施，每次最多按依赖顺序询问三个问题并给出建议。用户确认后只在 `repository-docs/personal/` 创建范围匹配的个人补充；它不得弱化、覆盖或冲突于仓库文档。适用文档契约的实质变化遵循与其他任务契约相同的版本门禁。
...
在任一阶段开始实质工作前，Agent 都会检查宿主当前可用的 Skill、工具、MCP Server 和 Agent，并自主调用任何能提供实质帮助的能力。TaskFlow 不限定名称、提供方、调用链、能力类别或数量。所有输出都先审阅再纳入；存在 `plan.md` 时，只记录实际使用的能力、用途和已纳入的结论。
