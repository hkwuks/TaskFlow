...
When explicitly authorized to create a missing repository-owned document, reuse an established conventional filename or otherwise default to `README.md`, `CONTRIBUTING.md`, `CODE_STYLE.md`, or `ROADMAP.md` for those classes. Create it at the conventional source location even when the directory is not yet a Git repository. If legacy copies or symbolic links exist under `repository-docs/`, report them, stop using them, and index the authoritative source path; do not migrate or delete them without explicit user authorization.
...
When a task needs a rule not supplied by the repository, pause implementation and guide the user to define a scoped personal supplement: a person's development rules or work habits, not repository-owned policy. Ask no more than three dependency-ordered questions per turn, with a recommendation for each; ask only applicable categories (development process, code, commits/PR, then design by default). A repository-owned source document may be created or changed only with explicit user authorization.
